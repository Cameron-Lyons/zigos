...
5. `src/native/session/service_bootstrap.zig` launches the ordered contract services declared in `src/native/session/service_contracts.zig`, binds their interfaces into `src/native/kernel_api/service_registry.zig`, and attaches driver authority for device-backed services.
