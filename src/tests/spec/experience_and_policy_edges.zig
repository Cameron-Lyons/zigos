...
pub fn permissionReviewsAndSharingStayScopedAndInspectable() !void {
    ...
    const review_session = try permission_review.initSession(app_task.id, &bundle, &decisions);
    var grants_buffer: [permission_review.MAX_REVIEW_DECISIONS]policy_mediation.UserGrant = undefined;
    const grants = permission_review.decisionsToGrants(&review_session, 50, &grants_buffer);
    ...
    const rendered_summary = try permission_review.renderToBuffer(&summary_buffer, &review_session);
