...
const Entry = service_entry.Main(@as(runtime.ServiceKind, @enumFromInt(build_options.service_kind)));
pub const Descriptor = Entry.Descriptor;
pub const panic = Entry.panic;

export var zigos_userspace_descriptor: Descriptor align(@alignOf(Descriptor)) linksection(@import("userspace_descriptor").ELF_SECTION_NAME) = Entry.initDescriptor();
