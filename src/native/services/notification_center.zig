...
pub const Center = struct {
    ...
    pub fn post(self: *Center, request: PostRequest) Error!*Notification {
        ...
        const slot = self.reserveNotificationSlot(notification_id) orelse return error.NotificationTableFull;
    ...
    fn reserveNotificationSlot(self: *Center, notification_id: u64) ?*NotificationSlot {
        if (self.notifications.reserve(notification_id)) |slot| return slot;
        ...
        return self.notifications.reserve(notification_id);
    }
    ...
    fn hasReusableNotificationSlot(self: *const Center) bool {
        if (self.notifications.countInUse() < MAX_NOTIFICATIONS) return true;
