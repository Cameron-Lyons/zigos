...
pub const Service = struct {
    ...
    pub fn authorize(
        ...
        const slot = self.slots.reserve(delegation_id) orelse return error.DelegationTableFull;
        errdefer _ = self.slots.remove(delegation_id);
    ...
    pub fn killSwitch(
        ...
        for (&self.slots.slots) |*slot| {
            if (!shouldRevokeForGeneration(slot, minimum_generation)) continue;
            revoked_count += 1;
            if (ledger) |active_ledger| {
...
fn shouldRevokeForGeneration(slot: *const Slot, minimum_generation: u32) bool {
    return slot.in_use and
        !slot.delegation.revoked and
        slot.delegation.delegation_generation < minimum_generation;
}
