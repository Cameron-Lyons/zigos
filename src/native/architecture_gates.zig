...
pub const indexed_hot_path_tables = .{
    ...
    .task_runtime = .{
        ...
        .reuses_resolved_termination_tasks = task_runtime.TERMINATION_TASK_INDEX_RELOOKUPS == 0 and
            ...
            task_runtime.RESOLVED_TERMINATION_HANDLE_DERIVATIONS == 0 and
            ...
            @hasDecl(task_runtime.Runtime, "terminateResolvedTask") and
        ...
        .reuses_resolved_delegation_sources = capability.DELEGATION_SOURCE_INDEX_RELOOKUPS == 0 and
            @sizeOf(capability.ResolvedCapability) <= capability.RESOLVED_CAPABILITY_SIZE_CEILING_BYTES,
        ...
        .terminates_resolved_tasks_without_handles = native_kernel.TASK_TERMINATE_HANDLE_DERIVATIONS == 0,
        ...
        .resolves_task_lifecycle_transitions_once = task_lifecycle_service.TRANSITION_TASK_INDEX_LOOKUPS == 1 and
            task_lifecycle_service.TRANSITION_HANDLE_DERIVATIONS == 0,
    ...
    .driver_service = .{
        ...
        .stores_compact_record_metadata = driver_service.COMPACT_DRIVER_RECORD_METADATA and
            driver_service.COMPACT_DRIVER_SIGNER_FINGERPRINTS and
            @FieldType(driver_service.DriverRecord, "signer_fingerprint") == driver_service.SignerFingerprint and
            !@hasField(driver_service.DriverRecord, "signer_len") and
            !@hasField(driver_service.DriverRecord, "signer"),
    ...
    .network_driver_task = .{
        ...
        .drains_full_receive_queue_without_copy = network_driver_task.DRAINS_FULL_RECEIVE_QUEUE_WITHOUT_COPY and
            network_driver_task.bounded_metadata_layout.receive_overflow_scratch_bytes == 0,
    },
    .device_broker = .{
    ...
    .compositor_session = .{
        ...
        .resolves_existing_surfaces_once = compositor_session.STEADY_SURFACE_PRIMARY_INDEX_LOOKUPS == 1 and
            compositor_session.STEADY_SURFACE_TASK_INDEX_LOOKUPS == 0,
        .reuses_resolved_surface_slots_for_lifecycle = compositor_session.SURFACE_LIFECYCLE_BACKING_RELOOKUPS == 0 and
            compositor_session.SURFACE_REMOVAL_TASK_INDEX_LOOKUPS == 1,
    ...
    .sync_transport = .{
        ...
        .heap_backs_endpoint_table_on_freestanding = sync_transport.HEAP_BACKED_ENDPOINT_TABLE_ON_FREESTANDING and
            sync_transport.transport_endpoint_table_layout.heap_backs_table_on_freestanding and
            sync_transport.transport_endpoint_table_layout.freestanding_handle_size_bytes <= @sizeOf(?*anyopaque) and
            sync_transport.transport_endpoint_table_layout.freestanding_backing_size_bytes == endpoint.FREESTANDING_TABLE_SIZE_CEILING_BYTES and
            sync_transport.transport_endpoint_table_layout.freestanding_resident_savings_bytes ==
    ...
    .sync_service = .{
        .initializes_default_service_state_in_place = sync_service.IN_PLACE_DEFAULT_SERVICE_INITIALIZATION and
            @hasDecl(sync_service.Service, "initInto"),
        ...
        .overwrites_reused_overlay_session_slots = sync_service.OVERWRITES_REUSED_SESSION_SLOTS and
            @hasDecl(@FieldType(sync_service.Service, "overlay_sessions"), "reserveHandleForOverwrite") and
            @hasDecl(@FieldType(sync_service.Service, "overlay_sessions"), "replaceIndexForOverwrite"),
        .uses_generational_overlay_session_ids = sync_service.GENERATIONAL_OVERLAY_SESSION_IDS and
            @sizeOf(sync_service.OverlaySessionHandle) == @sizeOf(u64) and
            @hasDecl(@FieldType(sync_service.Service, "overlay_sessions"), "getByHandle"),
    ...
    .storage_volume = .{
        ...
        .derives_object_model_version_ids_from_canonical_head = object_store.DERIVES_OBJECT_MODEL_VERSION_IDS_FROM_CANONICAL_HEAD and
            !@hasField(object_store.ObjectRecord, "snapshot_state") and
            !@hasField(object_store.ObjectRecord, "sync_state"),
        .derives_object_model_counters_from_version_count = object_store.DERIVES_OBJECT_MODEL_COUNTERS_FROM_VERSION_COUNT,
        .derives_object_policy_and_recovery_from_canonical_data = object_store.DERIVES_OBJECT_POLICY_AND_RECOVERY_FROM_CANONICAL_DATA and
            !@hasField(object_store.ObjectRecord, "sharing_policy") and
            !@hasField(object_store.ObjectRecord, "recovery_history"),
        .derives_object_provenance_from_canonical_versions = object_store.DERIVES_OBJECT_PROVENANCE_FROM_CANONICAL_VERSIONS and
        ...
        .packs_blob_slot_membership_into_state = object_store.PACKS_BLOB_SLOT_MEMBERSHIP_INTO_STATE and
            !@hasField(object_store.BlobSlot, "in_use") and
        ...
        .elides_unused_object_store_slot_generations = object_store.ELIDES_UNUSED_OBJECT_STORE_SLOT_GENERATIONS and
            ...
            @sizeOf(object_store.Store) <= object_store.STORE_SIZE_CEILING_BYTES,
        .derives_object_store_arena_keys_from_payloads = object_store.DERIVES_OBJECT_STORE_ARENA_KEYS_FROM_PAYLOADS and
    ...
    .supervisor = .{
        ...
        .stores_compact_diagnostic_events = @sizeOf(supervisor.DiagnosticEvent) <= supervisor.DIAGNOSTIC_EVENT_SIZE_CEILING_BYTES and
            supervisor.OMITS_UNOBSERVED_DIAGNOSTIC_SEQUENCES and
            !@hasField(supervisor.DiagnosticEvent, "sequence") and
            !@hasField(supervisor.Supervisor, "next_diagnostic_sequence"),
