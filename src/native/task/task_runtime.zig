...
pub const UserImageByteLength = model.UserImageByteLength;
pub const UserStackByteLength = model.UserStackByteLength;
...
test "sparse checkpoints preserve cross-page slot order and retire only live address spaces" {
    ...
    try std.testing.expect(runtime.findByHandle(stale_task_handle, removed_task_id) == null);
