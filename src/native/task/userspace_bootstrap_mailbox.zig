...
pub const Mailbox = extern struct {
    version: u16 = VERSION,
    phase: u8 = @intFromEnum(Phase.boot),
...
pub fn packCounter(phase: Phase, detail: Detail, pulse: u16) u32 {
    return (@as(u32, @intFromEnum(phase)) << 24) |
        (@as(u32, @intFromEnum(detail)) << 16) |
        @as(u32, pulse);
}

pub fn phaseFromCounter(counter: u32) Phase {
    return @enumFromInt(@as(u8, @truncate(counter >> 24)));
}

test "mailbox counter encoding preserves phase and detail" {
    const counter = packCounter(.steady, .network, 42);
    try @import("std").testing.expectEqual(Phase.steady, phaseFromCounter(counter));
