...
const ConflictScopeLookup = sync_indexes.ConflictScopeLookup;
...
const InboundTransportTargetIndex = sync_indexes.InboundTransportTargetIndex;
...
pub fn ServiceWith(comptime config: ServiceConfig) type {
    ...
    return struct {
        ...
        inbound_transport_target_index: InboundTransportTargetIndex = InboundTransportTargetIndex.init(),
        ...
        pub fn configureOverlay(
            ...
            var overlay = if (existing_slot_index) |slot_index|
                self.stateConst().overlays[slot_index].overlay
            else
                zeroOverlay();
            ...
            const slot_index = existing_slot_index orelse self.allocateOverlayIndex() orelse return error.OverlayTableFull;
            if (overlay.id == 0) {
        ...
        pub fn resolveConflict(
            ...
            self.removeConflictIndexes(&conflict, slot_index);
            slot.* = .{};
            try self.checkpoint();
        ...
        pub fn recordConflict(
            ...
            const slot = &self.state().conflicts[slot_index];
            if (slot.in_use) self.removeConflictIndexes(&slot.conflict, slot_index);
            slot.in_use = true;
        ...
        fn countConflictsFor(self: *const Self, workspace_id: u64, device_id: principal.PrincipalId) usize {
            var count: usize = 0;
            var slot_index = self.conflict_scope_index.head(conflictScopeIndexKeyForLookup(workspace_id, device_id));
            while (slot_index != indexed_arena.no_index) {
                _ = self.conflictScopeIndexSlot(slot_index, workspace_id, device_id);
                count += 1;
                slot_index = self.conflict_scope_index.next(slot_index);
            }
            return count;
        }
        ...
        fn clearConflictsFor(self: *Self, workspace_id: u64, device_id: principal.PrincipalId) Error!bool {
            ...
            var slot_index = self.conflict_scope_index.head(conflictScopeIndexKeyForLookup(workspace_id, device_id));
            while (slot_index != indexed_arena.no_index) {
                const next_slot_index = self.conflict_scope_index.next(slot_index);
                _ = self.conflictScopeIndexSlot(slot_index, workspace_id, device_id);
                const slot = &self.state().conflicts[slot_index];
                self.removeConflictIndexes(&slot.conflict, slot_index);
                slot.* = .{};
                cleared = true;
        ...
        fn setReplicaVersionForPathHash(
            ...
            entry.object_id = object_store.ids.raw(object_id);
            entry.version_id = object_store.ids.raw(version_id);
            ...
            if (inserted) self.appendReplicaScopeIndex(&slot.entry, slot_index);
            self.resident().markDirty();
        }

        fn replicaWorkspaceGeneration(self: *const Self, workspace_id: u64, device_id: principal.PrincipalId) u32 {
            var generation: u32 = 0;
            var slot_index = self.replica_scope_index.head(replicaScopeIndexKeyForLookup(workspace_id, device_id));
            while (slot_index != indexed_arena.no_index) {
                ...
                slot_index = self.replica_scope_index.next(slot_index);
            }
            return generation;
        }
        ...
        fn rebuildReplicaScopeIndex(self: *Self) void {
            self.replica_scope_index.reset();
            for (self.stateConst().replica_entries, 0..) |slot, slot_index| {
                if (!slot.in_use) continue;
                self.appendReplicaScopeIndex(&slot.entry, slot_index);
            }
        }
        ...
        fn rebuildLookupIndexes(self: *Self) void {
            ...
            self.rebuildConflictScopeIndex();
        ...
        fn rebuildConflictScopeIndex(self: *Self) void {
            self.conflict_scope_index.reset();
            for (self.stateConst().conflicts, 0..) |slot, slot_index| {
                if (!slot.in_use) continue;
                if (!self.conflict_scope_index.append(conflictScopeIndexKeyFor(&slot.conflict), slot_index)) {
                    native_util.impossibleByInvariant("conflict scope index rebuild exceeded capacity");
                }
            }
        }
        ...
        fn rebuildInboundTransportTargetIndex(self: *Self) void {
            self.inbound_transport_target_index.reset();
            for (self.stateConst().inbound_transport_frames, 0..) |slot, slot_index| {
                if (!slot.in_use) continue;
                if (!self.inbound_transport_target_index.append(inboundTransportTargetIndexKeyFor(&slot.frame), slot_index)) {
                    native_util.impossibleByInvariant("inbound transport target index rebuild exceeded capacity");
                }
            }
        }
        ...
        fn replicaScopeIndexKeyFor(entry: *const ReplicaEntry) u64 {
            return sync_indexes.replicaScopeIndexLookupKey(entry.workspace_id, entry.device_id);
        }

        fn replicaScopeIndexKeyForLookup(workspace_id: u64, device_id: principal.PrincipalId) u64 {
            return sync_indexes.replicaScopeIndexLookupKey(workspace_id, device_id);
        }

        fn appendReplicaScopeIndex(self: *Self, entry: *const ReplicaEntry, slot_index: usize) void {
            if (!self.replica_scope_index.append(replicaScopeIndexKeyFor(entry), slot_index)) {
                native_util.impossibleByInvariant("replica scope index insert exceeded capacity");
            }
        }
        ...
        fn conflictScopeIndexKeyFor(conflict: *const ConflictRecord) u64 {
            return sync_indexes.conflictScopeIndexLookupKey(conflict.workspace_id, conflict.device_id);
        }

        fn conflictScopeIndexKeyForLookup(workspace_id: u64, device_id: principal.PrincipalId) u64 {
            return sync_indexes.conflictScopeIndexLookupKey(workspace_id, device_id);
        }
        ...
        fn insertConflictIndexes(self: *Self, conflict: *const ConflictRecord, slot_index: usize) void {
            ...
            if (!self.conflict_scope_index.append(conflictScopeIndexKeyFor(conflict), slot_index)) {
                native_util.impossibleByInvariant("conflict scope index insert exceeded capacity");
            }
        }

        fn removeConflictIndexes(self: *Self, conflict: *const ConflictRecord, slot_index: usize) void {
            ...
            _ = self.conflict_scope_index.remove(conflictScopeIndexKeyFor(conflict), slot_index);
        }

        fn conflictScopeIndexSlot(
            self: *const Self,
            slot_index: usize,
            workspace_id: u64,
            device_id: principal.PrincipalId,
            ...
            if (!sync_indexes.conflictScopeSlotMatches(.{ .workspace_id = workspace_id, .device_id = device_id }, slot)) {
        ...
        fn inboundTransportTargetIndexKeyFor(frame: *const TransportFrame) u64 {
            return sync_indexes.inboundTransportTargetIndexLookupKey(frame.workspace_id, frame.target_device);
        }

        fn inboundTransportTargetIndexKeyForLookup(workspace_id: u64, target_device: principal.PrincipalId) u64 {
            return sync_indexes.inboundTransportTargetIndexLookupKey(workspace_id, target_device);
        }
        ...
        fn indexTransportFrameSlot(
            ...
            switch (queue_kind) {
                ...
                .inbound => {
                    ...
                    if (!self.inbound_transport_path_index.append(inboundTransportPathIndexKeyFor(&slot.frame), slot_index)) {
                        _ = self.inbound_transport_target_index.remove(inboundTransportTargetIndexKeyFor(&slot.frame), slot_index);
                        self.inbound_transport_duplicate_index.remove(inboundTransportDuplicateIndexKeyForFrame(&slot.frame));
                        return error.TransportQueueFull;
                    }
                    self.indexInboundTransportHighWaterSlot(slot, slot_index);
        ...
        fn removeInboundTransportFrameIndexes(self: *Self, frame: *const TransportFrame, slot_index: usize) InboundTransportScopeLookup {
            ...
            _ = self.inbound_transport_target_index.remove(inboundTransportTargetIndexKeyFor(frame), slot_index);
        ...
        fn copyTransportFrameSlotsForSince(
            ...
            return self.copyInboundTransportFrameSlotsForSince(workspace_id, target_device, min_frame_id, out);
        }
        ...
        fn inboundTransportFrameSlotCountFor(self: *const Self, workspace_id: u64, target_device: principal.PrincipalId) usize {
            ...
            while (slot_index != indexed_arena.no_index) {
                ...
                count += 1;
                slot_index = self.inbound_transport_target_index.next(slot_index);
            }
            return count;
        }

        fn copyInboundTransportFrameSlotsForSince(
            self: *const Self,
            workspace_id: u64,
            target_device: principal.PrincipalId,
            min_frame_id: u64,
