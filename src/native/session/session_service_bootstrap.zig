...
const StorageRestartProbe = struct {
    ...
    broker_revoke_rejected: bool = false,
    ...
    fn verifyBeforeRestart(self: *@This(), service_id: u64) bool {
        ...
        if (!writeRestartProbeStorage(service_id, storage_restart_scratch_lba, expected[0..])) return false;
    ...
    fn verifyPublishedBackendFailureChecks(
        ...
        var invalid_read: [storage_volume_mod.sector_size]u8 = undefined;
        @memset(invalid_read[0..], 0);
        self.timeout_propagated = !readRestartProbeStorage(
            service_id,
            storage_restart_scratch_lba + 1_048_576,
            invalid_read[0..],
        );
        ...
        self.partial_transfer_rejected = !writeRestartProbeStorage(
            service_id,
    ...
    pub fn activateAt(self: *@This(), driver: *const driver_service.DriverRecord, tick: u64) !driver_runtime_mod.ActivationRecord {
        ...
        if (driver.device_class == .storage_controller) {
            if (self.storage_probe) |probe| {
                if (!probe.rejectStaleAccessAfterGenerationChange(driver.service_id)) return error.StaleStorageAccessNotRejected;
            }
        }
...
pub fn proveStorageDriverRestartIo(
    ...
    if (!storage_probe.stale_authority_rejected or
        ...
        !storage_probe.timeout_propagated or
    ...
    common.printBootMarker(boot_markers.service_boot_storage_timeout_propagated);
    ...
    if (!storage_probe.verifyBrokerRevocationRecovery(state.services.storage_service.id, storage_driver, kernel_port) or
        !storage_probe.broker_revoke_rejected or
    ...
    common.printBootMarker(boot_markers.service_boot_storage_broker_revoke_rejected);
