...
pub fn run(context: *support.Context) support.Phase4State {
    support.common.printBootMarker("ZIGOS:PHASE4:BOOTSTRAP:START");
    var phase4_reloaded = false;
    if (storage_service_mod.Service.hasCachedPersistentState()) {
        support.common.printBootMarker("ZIGOS:PHASE4:PERSIST:CACHE_READY");
    } else {
        support.common.printBootMarker("ZIGOS:PHASE4:PERSIST:RESET_START");
        storage_service_mod.Service.resetPreparedState();
        support.common.printBootMarker("ZIGOS:PHASE4:PERSIST:RESET_DONE");
        if (storage_volume_mod.hasAttachedDevice()) {
            support.common.printBootMarker("ZIGOS:PHASE4:PERSIST:DEVICE_READY");
            support.common.printBootMarker("ZIGOS:PHASE4:PERSIST:LOAD_START");
            phase4_reloaded = storage_service_mod.Service.loadPreparedStateFromAttachedVolume();
            if (phase4_reloaded) {
                support.common.printBootMarker("ZIGOS:PHASE4:PERSIST:LOAD_OK");
            } else {
                support.common.printBootMarker("ZIGOS:PHASE4:PERSIST:LOAD_MISS");
            }
        } else {
            support.common.printBootMarker("ZIGOS:PHASE4:PERSIST:NO_DEVICE");
        }
    }
    ...
    support.common.printBootMarker("ZIGOS:PHASE4:BOOTSTRAP:DONE");
    support.common.printBootMarker("ZIGOS:PHASE4:SEED:BOOTSTRAP_READY");

    var notes_object_id = native_util.fnv1a64("workspace:notes");
    support.common.printBootMarker("ZIGOS:PHASE4:SEED:CHECK_START");
    if (context.storage_service_instance.findWorkspace(context.session_user, "notes-workspace") == null) {
        support.common.printBootMarker("ZIGOS:PHASE4:SEED:EMPTY");
        support.common.printBootMarker("ZIGOS:PHASE4:SEED:PUT_V1_START");
        ...
        support.common.printBootMarker("ZIGOS:PHASE4:SEED:PUT_V1_METADATA_OK");
        ...
        _ = context.storage_service_instance.store.putVersionRef(&notes_v1_request) catch unreachable;
        support.common.printBootMarker("ZIGOS:PHASE4:SEED:PUT_V1_STORE_DONE");
        if (context.storage_service_instance.workspaces.snapshots[0].in_use) {
            support.common.printBootMarker("ZIGOS:PHASE4:SEED:SNAPSHOT_DIRTY_BEFORE_CHECKPOINT");
        }
        if (context.storage_service_instance.workspaces.workspaces[0].in_use) {
            support.common.printBootMarker("ZIGOS:PHASE4:SEED:WORKSPACE_DIRTY_BEFORE_CHECKPOINT");
        }
        const notes_v1_before_checkpoint = support.latestInsertedVersion(context.storage_service_instance.store).?;
        notes_object_id = notes_v1_before_checkpoint.object_id;
        const notes_v1_version_id = notes_v1_before_checkpoint.id;
        support.common.printBootMarker("ZIGOS:PHASE4:SEED:PUT_V1_PRECHECK_OK");
        support.common.printBootMarker("ZIGOS:PHASE4:SEED:PUT_V1_DONE");
        const notes_v1 = support.latestInsertedVersion(context.storage_service_instance.store).?;
        ...
        const notes_v2_version_id = support.latestInsertedVersion(context.storage_service_instance.store).?.id;
        const blob_request = object_store_mod.PutRequest{
            .preferred_object_id = 920,
            .object_type = .blob,
            .payload = "blob-bytes",
            .metadata = object_store_mod.signMetadata(support.phase4_storage_signer, "blob", "application/octet-stream", .blob, "blob-bytes", 82) catch unreachable,
        };
        ...
        const inbox_collection_request = object_store_mod.PutRequest{
            ...
            .metadata = object_store_mod.signMetadata(support.phase4_storage_signer, "inbox", "application/zigos-collection", .collection, "notes,archive", 83) catch unreachable,
        };
        _ = context.storage_service_instance.putVersionRef(&inbox_collection_request) catch unreachable;
        const inbox_collection = support.latestInsertedVersion(context.storage_service_instance.store).?;
        const secret_request = object_store_mod.PutRequest{
            .preferred_object_id = 922,
            .object_type = .secret,
            .payload = "enc:workspace-secret",
            .metadata = object_store_mod.signMetadata(support.phase4_storage_signer, "secret", "application/zigos-secret", .secret, "enc:workspace-secret", 84) catch unreachable,
        };
        ...
        const cover_media_request = object_store_mod.PutRequest{
            ...
            .payload = "jpeg:cover",
            .metadata = object_store_mod.signMetadata(support.phase4_storage_signer, "cover", "image/jpeg", .media_asset, "jpeg:cover", 85) catch unreachable,
        };
        _ = context.storage_service_instance.putVersionRef(&cover_media_request) catch unreachable;
        const cover_media = support.latestInsertedVersion(context.storage_service_instance.store).?;
        const model_request = object_store_mod.PutRequest{
            .preferred_object_id = 924,
            .object_type = .model_artifact,
            .payload = "tiny-embed-v1",
            .metadata = object_store_mod.signMetadata(support.phase4_storage_signer, "embed", "application/zigos-model", .model_artifact, "tiny-embed-v1", 86) catch unreachable,
        };
        ...
        const event_request = object_store_mod.PutRequest{
            ...
            .metadata = object_store_mod.signMetadata(support.phase4_storage_signer, "events", "application/zigos-event-stream", .event_stream, "append:event-1", 87) catch unreachable,
        };
        ...
        const baseline_snapshot = context.storage_service_instance.snapshot(seeded_workspace.id, "baseline", support.phase4_workspace_signer) catch unreachable;
        ...
        context.storage_service_instance.exportSnapshotInto(seeded_workspace.id, baseline_snapshot.id, support.phase4_export_signer, context.export_package) catch unreachable;
    ...
    if (phase4_reloaded) {
        support.common.printBootMarker(boot_markers.phase4_persistence_reloaded);
    }

    support.common.printBootMarker("ZIGOS:PHASE4:RELOAD:NOTES_WORKSPACE:START");
    ...
    support.common.printBootMarker(boot_markers.phase4_reload_notes_workspace_done);
    const notes_workspace_id = notes_workspace.id;
    support.common.printBootMarker("ZIGOS:PHASE4:RELOAD:BASELINE:START");
    const baseline_snapshot = context.storage_service_instance.findSnapshot(notes_workspace_id, "baseline") orelse blk: {
        support.common.printBootMarker("ZIGOS:PHASE4:RELOAD:BASELINE:MISS");
        break :blk context.storage_service_instance.snapshot(notes_workspace_id, "baseline", support.phase4_workspace_signer) catch unreachable;
    };
    support.common.printBootMarker("ZIGOS:PHASE4:RELOAD:BASELINE:DONE");
    ...
    support.common.printBootMarker("ZIGOS:PHASE4:RELOAD:EXPORT_REFRESH:DONE");
    support.common.printBootMarker("ZIGOS:PHASE4:RELOAD:IMPORTED_WORKSPACE:START");
    const imported_workspace = context.storage_service_instance.findWorkspace(context.storage_service_principal, "imported-notes") orelse
        ...
        support.common.printBootMarker("ZIGOS:PHASE4:RELOAD:IMPORTED_WORKSPACE:MISS");
    ...
    support.common.printBootMarker(boot_markers.phase4_reload_imported_workspace_done);
    ...
    support.common.printBootMarker("ZIGOS:PHASE4:RELOAD:NOTES_ENTRY:DONE");
    const latest_notes_version_id = context.storage_service_instance.store.latestVersion(notes_object_id).?.id;
    support.common.printBootMarker(boot_markers.phase4_reload_latest_version_done);

    if (contract.serviceDescriptor(.storage_object).?.boundary == .userspace_service and
        context.storage_service_instance.store.latestVersion(notes_object_id) != null)
    {
        support.common.printBootMarker(boot_markers.phase4_object_store_ready);
    }
    if (context.storage_service_instance.store.objectCount() >= 7 and context.storage_service_instance.store.versionCount() >= 8) {
        support.common.printBootMarker("ZIGOS:PHASE4:OBJECT_TYPES:READY");
    }
    if (context.storage_service_instance.workspaces.hasAccess(notes_workspace_id, .{
        .principal_id = context.sync_service_principal,
        .wants_write = true,
        .wants_export = true,
        ...
        support.common.printBootMarker("ZIGOS:PHASE4:WORKSPACE:SHARING_OK");
    }
    if (notes_workspace.entry_count == 3 and notes_workspace.generation >= 1) {
        support.common.printBootMarker(boot_markers.phase4_workspace_transaction_ok);
    }
    if (std.mem.eql(u8, baseline_snapshot_signer, "zigos-workspace-key")) {
        support.common.printBootMarker("ZIGOS:PHASE4:SNAPSHOT:OK");
    }
    if (latest_notes_version_id != notes_entry.version_id) {
        support.common.printBootMarker("ZIGOS:PHASE4:RESTORE:OK");
    }
    if (notes_workspace.deleted_count != 0 and notes_entry.version_id != 0) {
        support.common.printBootMarker("ZIGOS:PHASE4:DELETE_RECOVERY:OK");
    }
    if ((context.storage_service_instance.resolve(imported_workspace_id, "documents/notes.md") catch unreachable).version_id == notes_entry.version_id) {
        support.common.printBootMarker("ZIGOS:PHASE4:EXPORT_IMPORT:OK");
    }
    ...
    if (context.supervisor.recordCrash(context.storage_service_id, 94, 0x53)) {
        ...
        if (context.supervisor.hasDiagnostic(context.storage_service_id, .restart_completed) and
            (context.storage_service_instance.resolve(notes_workspace_id, "documents/notes.md") catch unreachable).version_id == notes_entry.version_id)
        {
            support.common.printBootMarker(boot_markers.phase4_storage_service_recovered);
        }
    }
    ...
    {
        support.common.printBootMarker(boot_markers.phase4_file_bridge_derived);
    }
    ...
    if (context.storage_service_instance.bridgeResolve(.{
        ...
        if (err == error.CapabilityRequired) {
            support.common.printBootMarker(boot_markers.phase4_path_authority_deprecated);
        }
    }
