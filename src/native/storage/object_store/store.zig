...
const ChunkState = packed struct(u16) {
    payload_len: ChunkPayloadLength = 0,
    arena_in_use: bool = false,
    reserved: u2 = 0,
};
...
pub const STORE_SIZE_CEILING_BYTES: usize = 1_311_664;
...
comptime {
    ...
    if (MAX_CHUNK_BYTES > std.math.maxInt(ChunkPayloadLength)) {
        @compileError("object chunk capacity exceeds packed chunk metadata");
    }
...
pub const ChunkRecord = struct {
    ...
    pub inline fn payloadLen(self: *const ChunkRecord) usize {
        return @intCast(self.state.payload_len);
    }
    ...
    fn writePayload(self: *ChunkRecord, bytes: []const u8) Error!void {
        ...
        self.state.payload_len = @intCast(bytes.len);
    }

    fn releasePayload(self: *ChunkRecord) void {
        if (comptime heap_backed_chunk_payloads) {
            if (self.payload) |payload| {
                @memset(payload[0..self.payloadLen()], 0);
...
pub const ChunkSlot = struct {
    ...
    pub inline fn arenaInUse(self: *const ChunkSlot) bool {
        return self.chunk.state.arena_in_use;
    }

    pub inline fn setArenaInUse(self: *ChunkSlot, in_use: bool) void {
        self.chunk.state.arena_in_use = in_use;
    }
};

comptime {
    if (@sizeOf(ChunkRecord) > CHUNK_RECORD_SIZE_CEILING_BYTES) {
        @compileError("object store chunk records exceed their compact layout");
    }
    if (@sizeOf(ChunkSlot) > CHUNK_SLOT_SIZE_CEILING_BYTES) {
        @compileError("object store chunk slots exceed their compact layout");
    }
}
...
pub fn StoreWith(comptime config: StoreConfig) type {
    ...
    return struct {
        ...
        const BlobArena = indexed_arena.PagedIndexedArenaWithoutHandles(BlobSlot, BLOB_PAGE_SIZE, STORE_BLOB_PAGE_COUNT, STORE_BLOB_INDEX_CAPACITY, blobSlotIndexId);
        const ChunkArena = indexed_arena.PagedIndexedArenaWithoutHandles(ChunkSlot, CHUNK_PAGE_SIZE, STORE_CHUNK_PAGE_COUNT, STORE_CHUNK_INDEX_CAPACITY, chunkSlotIndexId);
        ...
        pub const VersionChunkCursor = struct {
            ...
            pub fn next(self: *VersionChunkCursor) Error!?PayloadChunk {
                ...
                if (!chunk_slot.arenaInUse()) return error.BlobNotFound;
        ...
        pub fn blobChunkRef(self: *const Self, blob_record: *const BlobRecord, chunk_index: usize) Error!ChunkRef {
            ...
            return .{
                .address = chunk_record.address,
                .payload_len = @intCast(chunk_record.payloadLen()),
            };
        }
        ...
        fn checkedBlobChunk(self: *const Self, blob_record: *const BlobRecord, chunk_index: usize) Error!*const ChunkRecord {
            if (chunk_index >= blob_record.chunkCount() or chunk_index >= MAX_BLOB_CHUNKS) return error.CorruptBlob;
        ...
        fn putChunkPrepared(self: *Self, address: ChunkAddress, payload: []const u8) Error!usize {
            if (self.chunkSlotIndex(address)) |slot_index| {
                ...
                if (slot.chunk.payloadLen() != payload.len or !std.mem.eql(u8, slot.chunk.chunkSlice(), payload)) return error.CorruptBlob;
                return slot_index;
            }
...
pub fn chunkCountForPayloadLen(payload_len: usize) usize {
    return payload_len / MAX_CHUNK_BYTES + @intFromBool(payload_len % MAX_CHUNK_BYTES != 0);
}

pub fn chunkRefsMatchPayloadLen(payload_len: usize, chunk_refs: []const ChunkRef) bool {
    if (chunk_refs.len != chunkCountForPayloadLen(payload_len)) return false;
    var remaining = payload_len;
    for (chunk_refs) |chunk_ref| {
        const expected_len = @min(remaining, MAX_CHUNK_BYTES);
        if (chunk_ref.payload_len != expected_len) return false;
...
fn buildChunkRefs(payload: []const u8, out: *[MAX_BLOB_CHUNKS]ChunkRef) Error!usize {
    if (payload.len > MAX_PAYLOAD_BYTES) return error.PayloadTooLarge;
    const chunk_count = chunkCountForPayloadLen(payload.len);
...
test "store reset scrubs live records before retaining arena capacity" {
    ...
    try chunk_slot.chunk.writePayload("secret");
