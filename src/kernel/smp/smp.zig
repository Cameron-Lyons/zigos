...
const ACPIHeader = acpi_tables.ACPIHeader;
...
fn parseACPI() void {
    const rsdp = acpi_tables.findRSDP() orelse {
        vga.print("ACPI RSDP not found\n");
        return;
    };

    const root = acpi_tables.selectRootTable(rsdp) orelse return;
    _ = parseRootTable(root);
}

fn parseRootTable(root: acpi_tables.RootTable) bool {
    for (0..acpi_tables.entryCount(root)) |i| {
        const table_addr = acpi_tables.readEntry(root, i);
...
fn parseMPTables() void {
    ...
    if (!acpi_tables.validateChecksum(@ptrCast(mp_config), mp_config.base_table_length)) return;
...
fn scanMPRange(start: usize, end: usize) ?*align(1) const MPFloatingPointer {
    ...
    while (addr + @sizeOf(MPFloatingPointer) <= end) : (addr += 16) {
        ...
        if (std.mem.eql(u8, &mp.signature, "_MP_") and acpi_tables.validateChecksum(@ptrCast(mp), mp.length * 16)) {
            return mp;
        }
    }
    return null;
