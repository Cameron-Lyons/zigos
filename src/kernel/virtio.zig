...
fn initDevice(pci_device: pci.PCIDevice) void {
    ...
    pci.writeConfigWord(pci_device.bus, pci_device.device, pci_device.function, 0x04,
        pci.readConfigWord(pci_device.bus, pci_device.device, pci_device.function, 0x04) | 0x06);
                ...
                if (bar_addr != 0) {
                    ...
                    switch (cfg_type) {
                        ...
                        VIRTIO_PCI_CAP_NOTIFY_CFG => {
                            ...
                            if (cap_len >= 20) {
                                dev.notify_off_multiplier = pci.readConfigDword(
                                    dev.pci_device.bus, dev.pci_device.device, 
                                    dev.pci_device.function, cap_offset + 16
                                );
                            }
                        },
